...
from monitor_switch import MonitorSwitch
...
for n in ft.nodes():
    if ft.nodes[n]['type'] == 'host':
        hosts.add(n)
controller = Controller(env, ft, 30)
...
for node_id in ft.nodes():
    ...
    '''
    if node['type'] == 'switch':
        node['device'] = MonitorSwitch(env, k, pir, buffer_size, element_id=f"{node_id}")
    else:
...
for flow_id in sample(all_flows.keys(), 1):
    ...
    print(all_flows[flow_id].pkt_sink.packets_received)
    print(all_flows[flow_id].pkt_sink.bytes_received)
